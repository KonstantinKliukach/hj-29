...
addItem.addEventListener('submit', addItemToCart, true)
...
function loadColors(response) {
  ...
  for (let color of response) {
    colorSnippet.insertAdjacentHTML('beforeend', `<div data-value="${color.type}" class="swatch-element color ${color.type}">
  <div class="tooltip">${color.title}</div>
  <input quickbeam="color" id="${color.type}" type="radio" name="color" value="${color.type}">
...
function loadSizes (response) {
  ...
  for (let size of response) {
    sizeSnippet.insertAdjacentHTML('beforeend', `<div data-value="${size.type}" class="swatch-element ${size.type} plain s">
...
function updateCart (cart) {
  ...
  cartSnippet.innerHTML = ''
  cartSnippet.insertAdjacentHTML('beforeend', `<div class="quick-cart-product quick-cart-product-static" id="${cart[0].id}" style="opacity: 1">
...
function addItemToCart () {
  ...
  sendCart(url.cart, formData).then((response) => {updateCart(response)
  })
}
...
function removeItemFromCart () {
  ...
  sendCart(url.removeCart, formData).then((response) => {updateCart(response)
  })
}
